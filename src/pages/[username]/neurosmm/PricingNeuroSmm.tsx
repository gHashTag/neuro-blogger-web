export const pricingNeuroSmm = [
  {
    title: '🧠 НейроВстреча',
    description: `🤝 Индивидуальная встреча с экспертом<br />
      💡 Обсуждение ваших проектов и идей<br />
      📈 Персональные рекомендации и стратегии<br />
      🔗 Интеграция ИИ с вашими проектами`,
    discountedPrice: 49999,
    price: 69998,
    discountedLink:
      'https://auth.robokassa.ru/merchant/Invoice/Hu3lmJjUSUimg_A53_QQ4g',
    link: 'https://auth.robokassa.ru/merchant/Invoice/CCj0r98FBE-GWMtU9Q5vAA',
  },
  {
    title: '🌐 НейроСмм',
    description: `📈 Полная автоматизация SMM<br />
      🤖 Аватар ведет блог за вас<br />
      📅 Планирование и автопостинг<br />
      💬 Автоматические ответы подписчикам<br />
      🌍 Глобальный охват и мульти-языковой контент<br />
      📊 Анализ и оптимизация контента<br />
      🔄 Ежедневный выход Reels<br />
      📈 Увеличение охватов и вовлеченности<br />
      🔧 Интеграция с вашими текущими платформами<br />
      ⭐️ 5000 на баланс бота`,
    discountedPrice: 149999,
    price: 179998,
    discountedLink:
      'https://auth.robokassa.ru/merchant/Invoice/newDiscountedLink',
    link: 'https://auth.robokassa.ru/merchant/Invoice/newLink',
  },
  // {
  //   title: '🦸🏼‍♂️ НейроМентор',
  //   description: `📚 Все из тарифа НейроБаза<br />
  //     🧑‍🏫 Индивидуальное обучение и наставничество<br />
  //     📅 Персональные занятия с гибким графиком<br />
  //     🔗 Интеграция с вашим проектом<br />
  //     🚀 СММ на автопилоте<br />
  //     ⭐️ 10000 на баланс бота<br />
  //     📈 Анализ и оптимизация контента<br />
  //     🌍 Глобальный охват и мульти-языковой контент<br />
  //     🎯 Персонализированные стратегии продвижения<br />
  //     🛠 Поддержка и сопровождение 24/7<br />
  //     🎥 Ежедневный выход Reels и Stories<br />
  //     💡 Доступ к эксклюзивным материалам и вебинарам`,
  //   discountedPrice: 199999,
  //   price: 239998,
  //   discountedLink:
  //     'https://auth.robokassa.ru/merchant/Invoice/zohLUE0VVUCUokmwRWeylg',
  //   link: 'https://auth.robokassa.ru/merchant/Invoice/Qyy295dzDkq_8w7mLOkFQw',
  // },
]
